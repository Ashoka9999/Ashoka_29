...
public class HomeController {
	...
	{
		//code to save user details in the database
		String model= new String("registersuccess");
		userService.saveUser(user);
		return model;
	}
	@PostMapping(value="/authenticate")  //user entered details
	public String authenticateuser(@ModelAttribute("user") User user, Model model)
	{
		...
		if(user1!=null && user.getPassword().equals(user1.getPassword())) 
		{
			...
			model.addAttribute(viewname, user1);
	...
	//@ModelAttribute it is for total form information
	//@RequestParam only for one id
